...
public class EnderChest extends ICommand {
    ...
    public void onPlayerCommand(Player sp, Command cmd, String label, String[] args) {
        if (MainAPI.isAdmin(sp) || sp.hasPermission("a_core.enderchest")) {
            sp.openInventory(sp.getEnderChest());
            sp.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SUCCESS) + "エンダーチェストを開きました。");
            return;
        }
